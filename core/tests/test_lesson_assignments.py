...
def test_assign_class_succeeds_as_planned(client_a, school_a, counselor_a, active_year, class_levels):
    lesson = _lesson(school_a, counselor_a, active_year)
    payload = {"lesson": lesson.id, "class_level": class_levels[0].id, "class_number": 3}
...
def test_update_assignment_persists(client_a, school_a, counselor_a, active_year, class_levels):
    ...
    resp = client_a.patch(f"/lessonAssignments/{assignment.id}/", {"class_number": 5}, format="json")
    assert resp.status_code == 200
...
@pytest.mark.django_db
def test_complete_sets_status_summary_and_default_date(client_a, school_a, counselor_a, active_year, class_levels):
    lesson = _lesson(school_a, counselor_a, active_year)
    assignment = factories.LessonClassAssignmentFactory(lesson=lesson, class_level=class_levels[0])

    resp = client_a.post(
        f"/lessonAssignments/{assignment.id}/complete/", {"summary": "סיכום השיעור"}, format="json"
    )
    assert resp.status_code == 200
...
@pytest.mark.django_db
def test_complete_preserves_existing_summary_when_omitted(client_a, school_a, counselor_a, active_year, class_levels):
    lesson = _lesson(school_a, counselor_a, active_year)
...
@pytest.mark.django_db
def test_complete_other_school_assignment_404(client_a, school_b, counselor_b, active_year, class_levels):
...
@pytest.mark.django_db
def test_assign_to_other_school_lesson_returns_500(client_a, school_b, counselor_b, active_year, class_levels):
    ...
    payload = {"lesson": other_lesson.id, "class_level": class_levels[0].id, "class_number": 1}
